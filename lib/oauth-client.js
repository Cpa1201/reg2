  ...
  async _extractWorkspaceFromConsentPage(consentUrl) {
    try {
      ...
      const pageStateDump = await this.page.evaluate(() => {
        ...
        try {
          if (window.__NEXT_DATA__) fragments.push(JSON.stringify(window.__NEXT_DATA__));
        } catch {}

        try {
          if (window.__remixContext) fragments.push(JSON.stringify(window.__remixContext));
        } catch {}

        try {
          if (window.__INITIAL_STATE__) fragments.push(JSON.stringify(window.__INITIAL_STATE__));
        } catch {}
      ...
      if (!workspaces.length) {
        this._log(
          `consent page workspace extraction returned empty: status=${response ? response.status() : 0} finalUrl=${this._previewText(this.page.url(), 140)}`,
        );
        return null;
      }
