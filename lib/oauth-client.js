...
class OAuthClient {
  ...
  _getOtpRetryPolicy() {
    const receiveAttempts = Math.max(1, Number(this.config.otpReceiveAttempts || 5));
    const resendMaxAttempts = Math.max(0, Number(this.config.otpResendMaxAttempts || 3));
    const pollIntervalSeconds = Math.max(1, Number(this.config.otpPollIntervalSeconds || 5));

    return {
      receiveAttempts,
      resendMaxAttempts,
      pollIntervalSeconds,
      waitTimeoutSeconds: receiveAttempts * pollIntervalSeconds,
    };
  }

  async _waitForOtpWithResends(email, mailbox, initialOtpRequestedAt, resendOtp) {
    const policy = this._getOtpRetryPolicy();
    let otpRequestedAt = initialOtpRequestedAt || Date.now();

    for (let resendIndex = 0; resendIndex <= policy.resendMaxAttempts; resendIndex += 1) {
      this._log(
        `等待邮箱验证码: 轮次=${resendIndex + 1}/${policy.resendMaxAttempts + 1}, ` +
        `最多轮询 ${policy.receiveAttempts} 次 (每次约 ${policy.pollIntervalSeconds}s)`,
      );

      const otpCode = await mailbox.waitForCode(
        email,
        policy.waitTimeoutSeconds,
        otpRequestedAt,
      );
      if (otpCode) {
        return otpCode;
      ...
      if (resendIndex >= policy.resendMaxAttempts) {
        break;
      }

      this._log(
        `邮箱连续 ${policy.receiveAttempts} 次未收到验证码，触发重发 ` +
        `${resendIndex + 1}/${policy.resendMaxAttempts}`,
      );
      otpRequestedAt = Date.now();
      const resendOk = await resendOtp(otpRequestedAt, resendIndex + 1);
      if (!resendOk) {
        this._log('重发验证码接口返回失败，仍继续下一轮等待...');
      }
    }
  ...
  async _submitWorkspaceAndOrg(consentUrl, deviceId) {
    ...
    if (!sessionData || !sessionData.workspaces?.length) {
      ...
      const extracted = await this._extractWorkspaceFromConsentPage(consentUrl, deviceId);
      if (extracted && Array.isArray(extracted.workspaces)) {
        sessionData = { workspaces: extracted.workspaces };
        consentSnapshot = extracted.snapshot || null;
      }
    }

    if (!sessionData?.workspaces?.length) {
      // 有些会话在 consent 可直接重定向到 callback（无需显式 workspace 选择）
      const directFollow = await this._followUrlForCode(consentUrl, consentUrl);
      if (directFollow?.code) {
        return {
          code: directFollow.code,
          state: this._extractFlowState(null, directFollow.finalUrl || consentUrl),
          issue: '',
        };
      }
      ...
      const issue = this._rememberOAuthIssue(
        'workspace 解析失败',
        consentSnapshot || { status: 200, url: consentUrl, headers: {}, text: '', errorMessage: 'missing workspace metadata' },
        directFollow?.issue || 'missing workspace metadata',
      );
  ...
  async _decodeOAuthSessionCookie() {
    try {
      ...
      const rawValue = String(sessionCookie.value || '').trim();
      const parts = rawValue.split('.').filter(Boolean);
      const candidates = new Set([
        rawValue,
        rawValue.startsWith('j:') ? rawValue.slice(2) : '',
        ...parts,
        parts[1] || '',
      ].filter(Boolean));

      for (const candidateRaw of candidates) {
        const attemptValues = [candidateRaw];
        try {
          const uriDecoded = decodeURIComponent(candidateRaw);
          if (uriDecoded && uriDecoded !== candidateRaw) {
            attemptValues.push(uriDecoded);
          }
        } catch {}

        for (const candidate of attemptValues) {
          ...
          const padded = trimmed + '='.repeat((-trimmed.length) % 4);
          try {
  ...
  _isLikelyWorkspaceId(value) {
    const text = String(value || '').trim();
    if (!text || text.length < 6 || text.length > 128) return false;
    if (!/^[a-z0-9][a-z0-9:_-]*$/i.test(text)) return false;
    if (/^(inbox|junkemail|deleteditems|all|personal|default|home)$/i.test(text)) return false;
    return true;
  }

  _extractWorkspaceCandidatesFromText(text) {
    const source = String(text || '');
    if (!source) return [];

    const chunks = [];
    const workspaceKeyword = /workspaces?|workspace_id|workspaceId|selectedWorkspace/gi;
    for (const match of source.matchAll(workspaceKeyword)) {
      const index = typeof match.index === 'number' ? match.index : 0;
      chunks.push(source.slice(Math.max(0, index - 400), Math.min(source.length, index + 24000)));
      if (chunks.length >= 6) break;
    }

    if (chunks.length === 0) {
      chunks.push(source.slice(0, Math.min(source.length, 32000)));
    }
    ...
    const push = (id, kind = '') => {
      ...
      if (!this._isLikelyWorkspaceId(normalizedId)) return;
      if (seen.has(normalizedId)) return;
      seen.add(normalizedId);
      workspaces.push(kind ? { id: normalizedId, kind } : { id: normalizedId });
    };

    for (const chunk of chunks) {
      for (const match of chunk.matchAll(/"id"\s*:\s*"([^"]+)"[\s\S]{0,220}"kind"\s*:\s*"([^"]+)"/gi)) {
        push(match[1], match[2]);
      }

      for (const match of chunk.matchAll(/"workspace(?:_id|Id)?"\s*:\s*"([^"]+)"/gi)) {
        push(match[1]);
      }

      for (const match of chunk.matchAll(/data-workspace-id=["']([^"']+)["']/gi)) {
        push(match[1]);
      }

      const workspaceSectionMatch = chunk.match(/"workspaces"\s*:\s*\[([\s\S]{0,20000})/i);
      if (workspaceSectionMatch) {
        for (const idMatch of workspaceSectionMatch[1].matchAll(/"id"\s*:\s*"([^"]+)"/gi)) {
          push(idMatch[1]);
        }
      }
    }
  ...
  async _probeWorkspaceFromApiEndpoints(consentUrl, deviceId = '') {
    const requestContext = this.page.context().request;
    const headers = {
    ...
    const endpoints = [
      `${OAUTH_ISSUER}/api/accounts/session`,
      `${OAUTH_ISSUER}/api/accounts/session-data`,
      `${OAUTH_ISSUER}/api/oauth/session`,
      `${OAUTH_ISSUER}/api/accounts/workspaces`,
    ];

    let lastSnapshot = { status: 0, url: consentUrl, headers: {}, text: '', errorMessage: 'workspace probe not started' };
    for (const endpoint of endpoints) {
      const { snapshot } = await this._requestContextGetWithNetworkRetry(
        requestContext,
        endpoint,
        {
          headers,
        ...
        'workspace-probe',
        this._previewText(endpoint, 120),
      );
      const invalidUrl404 = Number(snapshot?.status || 0) === 404
        && /invalid url/i.test(String(snapshot?.text || ''));
      if (!invalidUrl404 && snapshot) {
        lastSnapshot = snapshot;
      }

      const workspaces = this._extractWorkspaceCandidatesFromText(snapshot?.text || '');
      if (workspaces.length) {
        this._log(`workspace probe hit: ${this._previewText(endpoint, 90)} -> ${workspaces.map((item) => item.id).join(', ')}`);
        return { workspaces, snapshot };
      }
    }

    return { workspaces: [], snapshot: lastSnapshot };
  }

  async _extractWorkspaceFromConsentPage(consentUrl, deviceId = '') {
    let response = null;
    let snapshot = null;
    try {
      const maxAttempts = this._getOAuthNetworkRetryAttempts();
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
          await this._browserPause();
          response = await this.page.goto(consentUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
          gotoError = null;
          break;
        } catch (error) {
          gotoError = error;
          const errorSnapshot = this._buildErrorSnapshot(consentUrl, error);
          if (attempt < maxAttempts && this._isRetryableNetworkIssue(errorSnapshot)) {
            await this._waitForOAuthNetworkRetry('consent 导航', attempt, maxAttempts, errorSnapshot);
            continue;
          }
          throw error;
        }
      ...
      if (this._hasCloudflareMarker(html)) {
        this._log('consent 页面疑似命中 Cloudflare challenge，等待浏览器完成校验...');
        for (let i = 0; i < 3; i += 1) {
          await randomDelay(1800, 3200);
          await this.page.waitForLoadState('networkidle', { timeout: 8000 }).catch(() => {});
          html = await this.page.content().catch(() => '');
          if (!this._hasCloudflareMarker(html)) {
            this._log('consent 页面已通过 challenge，继续提取 workspace...');
            break;
          }
        }
      }
      const pageStateDump = await this.page.evaluate(() => {
        const fragments = [];
        const push = (value) => {
          if (!value) return;
          try {
            fragments.push(typeof value === 'string' ? value : JSON.stringify(value));
          } catch {}
        };

        try { push(window.__NEXT_DATA__); } catch {}
        try { push(window.__remixContext); } catch {}
        try { push(window.__INITIAL_STATE__); } catch {}
        try { push(window.__NUXT__); } catch {}
        try { push(window.__APOLLO_STATE__); } catch {}
        try { push(window.__PRELOADED_STATE__); } catch {}
        try { push(window.__APP_STATE__); } catch {}
        try { push(window.__BOOTSTRAP_DATA__); } catch {}
        ...
        try {
          const scripts = Array.from(document.querySelectorAll('script'))
            ...
            .slice(0, 40);
          if (scripts.length) {
            fragments.push(scripts.join('\n'));
          }
        } catch {}

        try {
          const attrs = Array.from(document.querySelectorAll('[data-workspace-id],[data-org-id]'))
            .map((el) => `${el.getAttribute('data-workspace-id') || ''}|${el.getAttribute('data-org-id') || ''}`)
            .filter(Boolean);
          if (attrs.length) {
            fragments.push(attrs.join('\n'));
          }
        } catch {}
        ...
        try {
          const resources = performance.getEntriesByType('resource')
            ...
            .filter(Boolean)
            .filter((name) => name.includes('/api/') || name.includes('workspace') || name.includes('consent'));
          if (resources.length) {
            fragments.push(resources.join('\n'));
          }
        } catch {}
      ...
      const workspaces = this._extractWorkspaceCandidatesFromText(
        [html, pageStateDump].filter(Boolean).join('\n'),
      );
      if (workspaces.length) {
        this._log(`consent page workspace candidates: ${workspaces.map((item) => item.id).join(', ')}`);
        return { workspaces, snapshot };
      }

      const probeResult = await this._probeWorkspaceFromApiEndpoints(consentUrl, deviceId);
      if (probeResult.workspaces.length) {
        return probeResult;
      }

      this._log(
        `consent page workspace extraction returned empty: status=${snapshot.status} finalUrl=${this._previewText(snapshot.url, 140)}`,
      );
      return { workspaces: [], snapshot: probeResult.snapshot || snapshot };
    } catch (error) {
      const fallbackSnapshot = this._buildErrorSnapshot(consentUrl, error);
      const probeResult = await this._probeWorkspaceFromApiEndpoints(consentUrl, deviceId).catch(() => null);
      if (probeResult?.workspaces?.length) {
        return probeResult;
      }
      return { workspaces: [], snapshot: probeResult?.snapshot || snapshot || fallbackSnapshot };
    }
  }
  ...
  async _handleOtpVerification(email, deviceId, mailbox, state, options = {}) {
    ...
    const sentinelToken = await buildSentinelToken(
      deviceId,
      'email_otp_validate',
      this.ua,
      this.secChUa,
    ...
    const otpCode = await this._waitForOtpWithResends(
      email,
      mailbox,
      initialOtpRequestedAt,
      async () => {
        this._log('OAuth OTP ????????...');
        return this._sendEmailOtp(
          deviceId,
          state.currentUrl || state.continueUrl || `${OAUTH_ISSUER}/email-verification`,
        );
      },
