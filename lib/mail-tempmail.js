...
function htmlToVisibleText(value) {
  ...
  // 去掉脚本/样式，避免误提取 CSS 色值（例如 #202123）
...
class MailTempMail extends BaseMailProvider {
  ...
  async waitForCode(email, timeout = 600, otpSentAt = Date.now()) {
    ...
    try {
      baselineMessages = await this._fetchMessages(mailbox);
    } catch (error) {
      this._log(`Initial Temp Mail fetch failed: ${error.message}`);
    }
    const baselineSignatures = new Set(
      baselineMessages.map((message) => this._messageSignature(message)),
    );

    const initialMatch = this._pickOtpMessage(baselineMessages, otpSentAt, {
      allowUnknownTimestamp: false,
    });
    if (initialMatch) {
    ...
    while (Date.now() < deadline) {
      ...
      let messages = [];
      try {
        messages = await this._fetchMessages(mailbox);
      } catch (error) {
        this._log(`Polled Temp Mail fetch failed: ${error.message}`);
        continue;
      }
  ...
  _pickOtpMessage(messages, otpSentAt, options = {}) {
    ...
    for (const message of messages) {
      ...
      const isBaselineMessage = baselineSignatures instanceof Set && baselineSignatures.has(signature);
      const freshnessOptions = {
        // 对 baseline 的历史快照：必须带可解析时间戳且时间新鲜
        // 对轮询中新出现的消息：允许无时间戳
        allowUnknownTimestamp: isBaselineMessage ? false : allowUnknownTimestamp,
      };

      if (!this._isMessageFresh(message, otpSentAt, freshnessOptions)) {
        continue;
      }
  ...
  _extractCodeFromMessage(message) {
    ...
    while (genericMatch) {
      ...
      // 过滤 CSS hex color 场景（如 #202123 / color:202123）
      if (prevChar === '#') {
  ...
  _isMessageFresh(message, otpSentAt, options = {}) {
    const { allowUnknownTimestamp = true } = options;
    ...
    if (!timestamp) {
      if (!allowUnknownTimestamp) {
        this._log(`Message skipped due to missing timestamp: rawTime="${rawTime || ''}"`);
      }
      return allowUnknownTimestamp;
    }
  ...
  _summarizeMessage(message) {
    ...
    for (const item of contentCandidates) {
      ...
      if (item.includes('<') && item.includes('>')) {
        const cleaned = htmlToVisibleText(item);
        if (cleaned) {
          contentFragments.push(cleaned);
        }
      } else {
        const normalized = normalizeText(item);
        if (normalized) {
          contentFragments.push(normalized);
        }
      }
    }

    const contentText = normalizeText(contentFragments.join(' ')) || fullText;
    return {
      from,
      subject,
      fullText,
