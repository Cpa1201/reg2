...
class ChatGPTClient {
  ...
  async submitAboutYou(firstName, lastName, birthdate, referer = null) {
    ...
    const aboutYouReferer = referer || `${AUTH}/about-you`;

    try {
      await this.page.goto(aboutYouReferer, { waitUntil: 'domcontentloaded', timeout: 20000 });
      this._log(`about_you preflight navigate ok: ${this.page.url()}`);
    } catch (e) {
      this._log(`about_you preflight navigate failed: ${e.message}`);
    }
    ...
    let sawRegistrationDisallowed = false;
    ...
    for (let payloadIndex = 0; payloadIndex < payloadVariants.length; payloadIndex += 1) {
      ...
      for (let retry = 1; retry <= 2; retry += 1) {
        ...
        if (!sentinelToken) {
          ...
          lastError = 'missing sentinel token';
          continue;
        }

        const headers = {
          'Accept': 'application/json',
          'Referer': aboutYouReferer,
          'Origin': AUTH,
          'Content-Type': 'application/json',
          'Sec-Fetch-Site': 'same-origin',
          'Sec-Fetch-Mode': 'cors',
        ...
        const errorCode = String(result?.data?.error?.code || '').trim();
        const errorMessage = String(result?.data?.error?.message || '').trim();
        const bodyPreview = (result.text || '').slice(0, 260);

        lastError = `HTTP ${result.status}${errorCode ? `/${errorCode}` : ''}`;
        if (errorCode === 'registration_disallowed') {
          sawRegistrationDisallowed = true;
        }
        this._log(
          `about_you submit failed: status=${result.status} code=${errorCode || '-'} msg=${errorMessage || '-'} body=${bodyPreview}`,
        );

        if (result.status === 429 && retry < 2) {
    ...
    if (sawRegistrationDisallowed) {
      return [false, 'HTTP 400/registration_disallowed'];
    }
    ...
    for (let step = 0; step < 12; step++) {
      ...
      if (this._isAboutYou(state)) {
        if (!stopBeforeAboutYouSubmission && !accountCreated) {
          let [ok, nextState] = await this.submitAboutYou(
            firstName,
            lastName,
            birthdate,
            state.currentUrl || state.continueUrl || `${AUTH}/about-you`,
          ...
          if (!ok) {
            const [fallbackFirstName, fallbackLastName] = generateRandomName();
            const fallbackBirthdate = generateRandomBirthday();
            this._log(
              `about_you 首次提交失败(${nextState})，尝试备用资料: ${fallbackFirstName} ${fallbackLastName} / ${fallbackBirthdate}`,
            );

            [ok, nextState] = await this.submitAboutYou(
              fallbackFirstName,
              fallbackLastName,
              fallbackBirthdate,
              state.currentUrl || state.continueUrl || `${AUTH}/about-you`,
